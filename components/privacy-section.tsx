"use client";

import { Shield, Lock, Smartphone } from "lucide-react";
import * as motion from "motion/react-client";
import { useTranslation } from "react-i18next";

export function PrivacySection() {
  const { t } = useTranslation();
  // For type safety
  // type PrivacyFeature = { title: string; desc: string };
  // const features = t('privacy.features', { returnObjects: true }) as PrivacyFeature[];
  return (
    <section className="w-full py-12 md:py-24 lg:py-32 relative" id="privacy">
      <div className="absolute inset-0 bg-gradient-to-t from-primary-50/50 to-white z-0"></div>
      <div className="container px-4 md:px-6 relative z-10">
        <motion.div
          className="flex flex-col items-center justify-center space-y-4 text-center"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
        >
          <div className="space-y-2">
            <h2 className="text-3xl font-bold tracking-tighter sm:text-5xl text-primary-400">
              {t("privacy.section_title")}
            </h2>
            <p className="max-w-[900px] text-primary-300/80 md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
              {t("privacy.section_desc")}
            </p>
          </div>
        </motion.div>
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 md:grid-cols-3 mt-12">
          {[
            <Shield key={1} className="h-8 w-8 text-primary-400" />,
            <Lock key={2} className="h-8 w-8 text-primary-400" />,
            <Smartphone key={3} className="h-8 w-8 text-primary-400" />,
          ].map((Icon, i) => {
            const features = t("privacy.features", {
              returnObjects: true,
            }) as Array<{
              title: string;
              desc: string;
            }>;
            return (
              <motion.div
                key={i}
                className="flex flex-col items-center space-y-2 text-center"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: 0.1 * (i + 1) }}
              >
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-primary-100">
                  {Icon}
                </div>
                <h3 className="text-xl font-bold text-primary-400">
                  {features[i]?.title}
                </h3>
                <p className="text-primary-300/80">{features[i]?.desc}</p>
              </motion.div>
            );
          })}
        </div>
      </div>
    </section>
  );
}
